...
<div class="row">
    <div class="col-lg-6">
        <div class="card card-primary">
            <div class="card-header">
                <h3 class="card-title">Texto Servicio</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            ...
            @if ($servicio==null)
               <form role="form"method="POST" action="{{route('servicio.store')}}">
                  @csrf
            @else
               <form role="form"method="POST" action="{{route('servicio.update',$servicio->id)}}">
                  @csrf
                  @method('PUT')
              ...
              <div class="card-body">
                <div class="form-group">
                  <label for="texto1">Texto 1:</label>
                    <input type="text" value="{{$servicio->texto1}}" class="form-control" name="texto1" id="texto1" placeholder="Introduzca texto1">
                </div>
                <div class="form-group">
                  <label for="texto2">Texto 2:</label>
                  <input type="text" class="form-control" value="{{$servicio->texto2}}" name="texto2" id="texto2" placeholder="Introduzca texto2">
                </div>
              </div>
              @else
              <div class="card-body">
    ...
    <div class="col-lg-6" >
        <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Imagen Servicios</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form role="form"method="POST" action="{{route('servicio.update',$servicio->id)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
              <div class="card-body">
                <div class="form-group">
                  ...
                  <div class="input-group">
                    ...
                    <div class="input-group">
                        <img src="{{asset("images/servicio/$servicio->imagen")}}" id="pre" class="imagencita mt-3" alt="">
                    </div>
                  </div>
                </div>
    ...
    <div class="col-12">
        ...
        <div class="col-sm-12"><table id="table" class="table table-striped table-responsive-xl" style="width:100%" role="grid" aria-describedby="example2_info">
            ...
            <tbody>
                ...
                @foreach ($servicio1 as $item )
                <tr role="row" class="odd">
                    <td class="" tabindex="0">{{$item->id}}</td>
                    <td>{{$item->texto1}}</td>
                    <td>{{$item->texto2}}</td>
                    <td><img src="{{asset("images/servicio/$item->imagen")}}" id="pre" class="imagencita1 mt-1 mb-1" alt=""></td>
                    <td class="sorting_1">
                        <form role="form" method="GET" action="{{route('servicio.show',$item->id)}}">
                        @csrf
                        <button type="submit" class="btn btn-block btn-success btn-sm"><span class="fas fa-edit"></span>Editar</button>
                        </form>
                    </td>
                    <td class="sorting_1">
                        <form role="form" class="eliminar" method="POST" action="{{route('servicio.destroy',$item->id)}}">
                        @csrf
                        @method('delete')
