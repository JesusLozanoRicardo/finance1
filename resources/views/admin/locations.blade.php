        ...
        <div class="card card-primary">
            ...
            @if ($locations==null)
               <form role="form"method="POST" action="{{route('locations.store')}}">
                  @csrf
            @else
               <form role="form"method="POST" action="{{route('locations.update',$locations->id)}}">
                  @csrf
                  @method('PUT')
              ...
              <div class="card-body">
                <div class="form-group">
                  <label for="texto1">Texto 1:</label>
                    <input type="text" value="{{$locations->texto1}}" class="form-control" name="texto1" id="texto1" placeholder="Introduzca texto1">
                </div>
                <div class="form-group">
                  <label for="texto2">Texto 2:</label>
                  <input type="text" class="form-control" value="{{$locations->texto2}}" name="texto2" id="texto2" placeholder="Introduzca texto2">
                </div>
                <div class="form-group">
                  <label for="texto3">Texto 3:</label>
                  <input type="text" class="form-control" value="{{$locations->texto3}}" name="texto3" id="texto3" placeholder="Introduzca texto3">
                </div>
              </div>
              @else
              <div class="card-body">
...
<div class="col-lg-6" >
    <div class="card card-primary">
        <div class="card-header">
          <h3 class="card-title">Imagen Location</h3>
        </div>
        <!-- /.card-header -->
        <!-- form start -->
        <form role="form"method="POST" action="{{route('locations.update',$locations->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
          <div class="card-body">
            <div class="form-group">
              ...
              <div class="input-group">
                ...
                <div class="input-group">
                    <img src="{{asset("images/location/$locations->imagen")}}" id="pre" class="imagencita mt-3" alt="">
                </div>
              </div>
            </div>
    ...
    <div class="col-12">
        ...
        <div class="col-sm-12"><table id="table" class="table table-striped table-responsive-xl" style="width:100%" role="grid" aria-describedby="example2_info">
            ...
            <tbody>
                ...
                @foreach ($locations1 as $item )
                <tr role="row" class="odd">
                    <td class="" tabindex="0">{{$item->id}}</td>
                    <td>{{$item->texto1}}</td>
                    <td>{{$item->texto2}}</td>
                    <td>{{$item->texto3}}</td>
                    <td><img src="{{asset("images/location/$item->imagen")}}" id="pre" class="imagencita1 mt-1 mb-1" alt=""></td>
                    <td class="sorting_1">
                        <form role="form" method="GET" action="{{route('locations.show',$item->id)}}">
                        @csrf
                        <button type="submit" class="btn btn-block btn-success btn-sm"><span class="fas fa-edit"></span>Editar</button>
                        </form>
                    </td>
                    <td class="sorting_1">
                        <form role="form" class="eliminar" method="POST" action="{{route('locations.destroy',$item->id)}}">
                        @csrf
                        @method('delete')
